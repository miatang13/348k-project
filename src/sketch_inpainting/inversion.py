...
InversionCallback = Callable[[
    StableDiffusionXLPipeline, int, T, dict[str, T]], dict[str, T]]
...
def _encode_text_sdxl(model: StableDiffusionXLPipeline, prompt: str) -> tuple[dict[str, T], T]:
    ...
    prompt_embeds, pooled_prompt_embeds, = _get_text_embeddings(
        prompt, model.tokenizer, model.text_encoder, device)
    prompt_embeds_2, pooled_prompt_embeds2, = _get_text_embeddings(
        prompt, model.tokenizer_2, model.text_encoder_2, device)
    prompt_embeds = torch.cat((prompt_embeds, prompt_embeds_2), dim=-1)
    text_encoder_projection_dim = model.text_encoder_2.config.projection_dim
    if model.__class__.__name__ == 'StableDiffusionXLInpaintPipeline':
        '''
        ...
        '''
        add_time_ids, _ = model._get_add_time_ids(original_size=(1024, 1024), crops_coords_top_left=(0, 0),
                                                  target_size=(1024, 1024), dtype=torch.float16,
                                                  text_encoder_projection_dim=text_encoder_projection_dim,
                                                  negative_original_size=(1024, 1024), negative_crops_coords_top_left=(0, 0),
                                                  negative_target_size=(
                                                      1024, 1024),
                                                  aesthetic_score=6.0,
    ...
    added_cond_kwargs = {
        "text_embeds": pooled_prompt_embeds2, "time_ids": add_time_ids}
...
def _encode_image(model: StableDiffusionXLPipeline, image: np.ndarray) -> T:
    ...
    latent = model.vae.encode(image.to(model.vae.device))[
        'latent_dist'].mean * model.vae.config.scaling_factor
    model.vae.to(dtype=torch.float16)
    return latent


def _next_step(model: StableDiffusionXLPipeline, model_output: T, timestep: int, sample: T) -> T:
    timestep, next_timestep = min(timestep - model.scheduler.config.num_train_timesteps //
                                  model.scheduler.num_inference_steps, 999), timestep
    alpha_prod_t = model.scheduler.alphas_cumprod[int(
        timestep)] if timestep >= 0 else model.scheduler.final_alpha_cumprod
    alpha_prod_t_next = model.scheduler.alphas_cumprod[int(next_timestep)]
    beta_prod_t = 1 - alpha_prod_t
    next_original_sample = (sample - beta_prod_t **
                            0.5 * model_output) / alpha_prod_t ** 0.5
    next_sample_direction = (1 - alpha_prod_t_next) ** 0.5 * model_output
    next_sample = alpha_prod_t_next ** 0.5 * \
        next_original_sample + next_sample_direction
    return next_sample
...
def _ddim_loop(model: StableDiffusionXLPipeline, z0, prompt, guidance_scale) -> T:
    all_latent = [z0]
    added_cond_kwargs, text_embedding = _encode_text_sdxl_with_negative(
        model, prompt)
...
def make_inversion_callback(zts, offset: int = 0) -> [T, InversionCallback]:
    ...
    def callback_on_step_end(pipeline: StableDiffusionXLPipeline, i: int, t: T, callback_kwargs: dict[str, T]) -> dict[str, T]:
        ...
        latents[0] = zts[max(offset + 1, i + 1)
                         ].to(latents.device, latents.dtype)
        return {'latents': latents}
    return zts[offset], callback_on_step_end


def make_inversion_inpaint_callback(zts, mask, offset: int = 0) -> [T, InversionCallback]:
    # Process the mask once outside the callback to ensure it is ready for use

    def callback_on_step_end(pipeline: StableDiffusionXLPipeline, i: int, t: T, callback_kwargs: dict[str, T]) -> dict[str, T]:
        # Original latents as cloned from the callback arguments
        init_latents_proper = callback_kwargs['latents']

        processed_mask = mask.to(init_latents_proper.dtype)
        # Apply mask: replace masked regions with alternative_latents and keep the rest as is
        masked_latents = processed_mask * init_latents_proper + \
            (1 - processed_mask) * init_latents_proper

        # Return the modified latents in the callback dictionary
        return {'latents': masked_latents}

    # Return initial latents at 'offset' and the callback function
    return zts[offset], callback_on_step_end
